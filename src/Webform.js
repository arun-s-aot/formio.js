...
export default class Webform extends NestedDataComponent {
  ...
  init() {
    ...
    this.on(
      'checkValidity',
      (data) => this.validate(data, { dirty: true, process: 'change' }),
      true,
    );
  ...
  showErrors(errors, triggerEvent) {
    ...
    if (errors.length) {
      ...
      const createListItem = (message, index) => {
        ...
        const keyOrPath =
          messageFromIndex?.formattedKeyOrPath ||
  ...
  onChange(flags, changed, modified, changes) {
    ...
    if (this.submitted) {
      ...
      const nonComponentServerErrors = _.filter(
        this.serverErrors || [],
      ...
      this.hook('beforeSubmit', { ...submission, component: options.component }, (err, data) => {
        ...
        this.hook('customValidation', { ...submission, component: options.component }, (err) => {
          ...
          if (this._form && this._form.action) {
            const method =
              submission.data._id && this._form.action.includes(submission.data._id)
                ? 'PUT'
                : 'POST';
            return Formio.makeStaticRequest(
              ...
              method,
              submission,
              this.formio ? this.formio.options : {},
            )
              .then((result) =>
                resolve({
                  submission: result,
